...
zk.ev.on("messages.upsert", async (m) => {
  ...
  let auto_reply_message = `Hello👋 @${senderNumber}, Am Hans-Md here How can Help you my friend 🤔?.`;
...
// AUTO_READ_MESSAGES: Automatically mark messages as read if enabled.
      ...
      if (! superUser && origineMessage === auteurMessage && conf.AUTO_BLOCK === 'yes') {
        zk.sendMessage(auteurMessage, {
          'text': "⚠️am blocking you because you have violated Keith policies⚠️!"
        });
...
if (texte && texte.startsWith('>')) {
  ...
  if (!superUser) {
    const menuText = `This command is only for the owner or Sir Keith to execute ⚠️`;
    ...
    async  function activateCrons() {
        ...
        zk.ev.on("connection.update", async (con) => {
            ...
            if (connection === "connecting") {
                console.log("ℹ️ Hans Connecting...");
            }
            else if (connection === 'open') {
                console.log("✅ Hans Connection Established! ☺️");
                console.log("--");
                await (0, baileys_1.delay)(200);
                console.log("------");
                await (0, baileys_1.delay)(300);
                console.log("------------------/-----");
                console.log("Hans is Online 🕸\n\n");
